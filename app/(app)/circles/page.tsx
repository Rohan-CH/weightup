...
function CirclesPageInner() {
  ...
  const [lbExercises, setLbExercises] = useState<{id: string, name: string}[]>([]);
  const [lbSelectedExercise, setLbSelectedExercise] = useState('');
  ...
  // ---- Leaderboard ----
  const initLbExercises = async () => {
    const { data: exData } = await supabase
      .from('exercises')
      .select('id, name')
      .order('name');
    if (exData) {
      setLbExercises(exData);
      if (exData.length > 0 && !lbSelectedExercise) setLbSelectedExercise(exData[0].id);
    }
  };

  useEffect(() => {
    if (view === 'circle' && circleTab === 'leaderboard') {
      initLbExercises();
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [view, circleTab]);
  ...
  useEffect(() => {
    if (view !== 'circle' || circleTab !== 'leaderboard' || members.length === 0) return;
    const memberIds = members.map(m => m.user_id);
    
    const fetchExerciseLeaderboard = async () => {
      if (!lbSelectedExercise) return;
      setLbLoading(true);
      const { data: logs } = await supabase
        .from('workout_logs')
      ...
      if (!logs) { setLbLoading(false); return; }
    ...
    const fetchPowerlifterLeaderboard = async () => {
      ...
      if (!plExercises || plExercises.length === 0) {
        setPowerlifterBoard([]); setLbLoading(false); return;
      }
      ...
      if (userIds.length === 0) { setPowerlifterBoard([]); setLbLoading(false); return; }
    ...
    if (lbTab === 'exercise') fetchExerciseLeaderboard();
    else if (lbTab === 'powerlifter') fetchPowerlifterLeaderboard();
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [view, circleTab, lbTab, lbSelectedExercise, members]);
    ...
    return (
      <div className="animate-fade-in-up">
        ...
        <div className="tabs" style={{ maxWidth: 480 }}>
          ...
          <button className={`tab ${circleTab === 'leaderboard' ? 'active' : ''}`} onClick={() => setCircleTab('leaderboard')}>
            <Trophy size={15} style={{ marginRight: 6, verticalAlign: 'middle' }} /> Leaderboard
          </button>
        </div>
        ...
        {circleTab === 'leaderboard' && (
          <div className="animate-fade-in-up" style={{ marginTop: 24 }}>
            ...
            <div className="tabs" style={{ maxWidth: 460, marginBottom: 24 }}>
              <button className={`tab ${lbTab === 'exercise' ? 'active' : ''}`} onClick={() => setLbTab('exercise')}>
                <Trophy size={15} style={{ marginRight: 6, verticalAlign: 'middle' }} /> Exercise
              </button>
              <button className={`tab ${lbTab === 'powerlifter' ? 'active' : ''}`} onClick={() => setLbTab('powerlifter')}>
            ...
            {lbTab === 'exercise' && (
              <>
                <div className="form-group" style={{ maxWidth: 320, marginBottom: 24 }}>
                  ...
                  <select
                    className="select"
                    value={lbSelectedExercise}
                    onChange={(e) => setLbSelectedExercise(e.target.value)}
                  >
                    {lbExercises.map(ex => (
